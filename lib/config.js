...
export async function loadConfig(cliOpts = {}) {
  ...
  if (globalConfig.profiles[profileToLoad]) {
    config = { ...config, activeProfile: profileToLoad, ...globalConfig.profiles[profileToLoad] };
    config = applyCliOverrides(config, cliOpts);
    return config;
  ...
  if (fs.existsSync(localPath)) {
    try {
      ...
      config = { ...config, activeProfile: localConfig.activeProfile, ...localConfig };
    } catch (err) {
      console.warn("⚠️ Failed to load local config:", err.message);
    }
  }

  return applyCliOverrides(config, cliOpts);
}


// Apply any CLI overrides to the config
function applyCliOverrides(config, cliOpts) {
  let newConfig = { ...config };
  if (cliOpts.prompt) newConfig.prompt = cliOpts.prompt;
  if (cliOpts.model) newConfig.model = cliOpts.model;
